...
DEVIATION_BASE_COHORTS = range(1982, 1988)
DEVIATION_BASE_LABEL = f"{DEVIATION_BASE_COHORTS[0]}-{DEVIATION_BASE_COHORTS[-1]} cohort average"
RELATIVE_BASE_COHORTS = range(1990, 1993)
RELATIVE_BASE_LABEL = f"{RELATIVE_BASE_COHORTS[0]}-{RELATIVE_BASE_COHORTS[-1]} cohort average"
GLOBAL_BASE_LABEL = "all-country, all-cohort average"
...
def load_data():
    ...
    df = df[df["cohort"] >= DISPLAY_MIN_COHORT]
    df = df[df["code"] != "UKR"]
    return df
...
def make_grid(df, order):
    ...
    for ax, country in zip(axes, countries):
        ...
        for cohort, row in subset.groupby("cohort"):
            row = row.sort_values("age")
            ax.plot(row["age"], row[column], color=cmap(norm(cohort)), alpha=0.6, linewidth=0.8)
        ax.set_title(
            country_title(
                country, subset["cohort"].min(), subset["cohort"].max(),
    ...
    fig.suptitle(f"Conditional age-specific fertility rates for {config['label']} births, by birth cohort and country")

    sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm)
    fig.colorbar(sm, ax=axes.tolist(), label="Birth cohort", shrink=0.6)

    save(fig, f"outputs/cond_asfr{order}_cohort_grid.png")
...
def make_deviation_grid(df, order):
    ...
    for ax, country in zip(axes, countries):
        ...
        baseline = subset[subset["cohort"].isin(DEVIATION_BASE_COHORTS)].groupby("age")[column].mean()
        if baseline.empty:
        ...
        for cohort, row in subset.groupby("cohort"):
            ...
            ax.plot(deviation.index, deviation.values, color=cmap(norm(cohort)), alpha=0.6, linewidth=0.8)
    ...
    save(fig, f"outputs/cond_asfr{order}_cohort_deviation_grid.png")
...
def make_relative_deviation_grid(df, order):
    ...
    for ax, country in zip(axes, countries):
        ...
        baseline = subset[subset["cohort"].isin(RELATIVE_BASE_COHORTS)].groupby("age")[column].mean()
        if baseline.empty:
        ...
        for cohort, row in subset.groupby("cohort"):
            ...
            ax.plot(relative_deviation.index, relative_deviation.values, color=cmap(norm(cohort)), alpha=0.6, linewidth=0.8)
        ax.axhline(0, color="black", linewidth=0.8, linestyle="--")
        ax.set_title(title, fontsize=9)
        ax.set_xlim(15, 45)
        ax.set_ylim(-1, 2)
    ...
    save(fig, f"outputs/cond_asfr{order}_cohort_relative_deviation_grid.png")
...
def make_relative_deviation_log_grid(df, order):
    ...
    for ax, country in zip(axes, countries):
        ...
        for cohort, row in subset.groupby("cohort"):
            row = row.sort_values("age").set_index("age")[column]
            ratio = row / baseline
            ax.plot(ratio.index, ratio.values, color=cmap(norm(cohort)), alpha=0.6, linewidth=0.8)
    ...
    save(fig, f"outputs/cond_asfr{order}_cohort_relative_deviation_log_grid.png")
...
def make_relative_deviation_log_global_grid(df, order):
    ...
    save(fig, f"outputs/cond_asfr{order}_cohort_relative_deviation_log_global_grid.png")
